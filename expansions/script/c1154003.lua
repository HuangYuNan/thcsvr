...
--
	c:EnableReviveLimit()
	aux.AddLinkProcedure(c,c1154003.lkcheck,2,2)
--
	...
	if tc:IsRelateToEffect(e) and tc:IsLocation(LOCATION_GRAVE) then
		if Duel.SelectYesNo(1-tp,aux.Stringid(1154003,2)) then
			...
			if Duel.GetLocationCount(1-tp,LOCATION_MZONE)>0 and Duel.IsPlayerCanSpecialSummonMonster(tp,1154004,0,0x4011,500,500,1,RACE_ZOMBIE,ATTRIBUTE_DARK) then
				...
				e1_10:SetReset(RESET_EVENT+0x1fe0000+EVENT_PHASE+PHASE_END+RESET_OPPO_TURN,2)
				e1_10:SetCondition(c1154003.con1_10)
				e1_10:SetOperation(c1154003.op1_10)
				e1_10:SetLabel(0)
				token:RegisterEffect(e1_10,true) 
...
function c1154003.cfilter2(c,e,tp)
	return c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:IsRace(RACE_ZOMBIE)
end
