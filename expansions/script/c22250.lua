--元素融合『板块』
function c22250.initial_effect(c)
	--activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_FUSION_SUMMON)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetTarget(c22250.target)
	e1:SetOperation(c22250.activate)
	c:RegisterEffect(e1)  
	--to gravee
	...
	e2:SetCost(c22250.thcost)
	e2:SetTarget(c22250.tg4)
	e2:SetOperation(c22250.op4)
	c:RegisterEffect(e2) 
end
...
function c22250.filter2(c,e,tp,m,f,chkf)
...
function c22250.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if ce~=nil then
		...
		mg3=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c22250.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg3,mf,chkf)
	end
...
function c22250.tg4(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local token1=Duel.CreateToken(tp,22131)
	local token2=Duel.CreateToken(tp,22134)
...
--
function c22250.op4(e,tp,eg,ep,ev,re,r,rp)
