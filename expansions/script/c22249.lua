--元素融合『侵染』
function c22249.initial_effect(c)
	--activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_FUSION_SUMMON)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetTarget(c22249.target)
	e1:SetOperation(c22249.activate)
	c:RegisterEffect(e1)  
	--to gravee
	...
	e2:SetCost(c22249.thcost)
	e2:SetTarget(c22249.tg4)
	e2:SetOperation(c22249.op4)
	c:RegisterEffect(e2) 
end
...
function c22249.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if ce~=nil then
		...
		mg3=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c22249.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg3,mf,chkf)
	end
...
function c22249.tg4(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local token1=Duel.CreateToken(tp,22131)
	local token2=Duel.CreateToken(tp,22132)
	local token3=Duel.CreateToken(tp,22162)
...
--
function c22249.op4(e,tp,eg,ep,ev,re,r,rp)
